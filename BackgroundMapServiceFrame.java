...
public class BackgroundMapServiceFrame1 implements Runnable {

	private BufferedImage serviceImg;
	private Player player;

	public BackgroundMapServiceFrame1(Player player) {
		this.player = player;
		try {
...
//	private void changeServiceFrame() {
//		try {
//			serviceImg = ImageIO.read(new File("images/Map_delService.jpg"));
//		} catch (IOException e) {
//			e.printStackTrace();
//		}
//	}
	...
	public void run() { // class가 Runnable이라서 자동 override
		...
		while (true) { // 무한루프
			...
			Color leftColor = new Color(serviceImg.getRGB(player.getX() + 15, player.getY() + 20));
			int leftColorInt = serviceImg.getRGB(player.getX() + 15, player.getY() + 20);

			Color rightColor = new Color(serviceImg.getRGB(player.getX() + pWidth, player.getY() + 20));
			int rightColorInt = serviceImg.getRGB(player.getX() + pWidth, player.getY() + 20);
